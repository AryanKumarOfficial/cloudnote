    ...
    return (
        <main className={"flex flex-col justify-center items-center min-h-screen gap-10"}>
            ...
            <section className={"flex flex-row justify-center items-center flex-wrap"}>
                {notes.length > 0 && notes.map((item: INote, index) => {
                    return (
                        <div key={index}
                ...
                {notes.length === 0 && <h1 className={"text-2xl font-semibold text-center"}>No Notes Found</h1>}

            </section>
            <Link href={"/notes/add"} id={"add-note"}
                  className={"bg-indigo-500 p-4 rounded text-white hover:bg-indigo-700 shadow-md transition-colors duration-500 font-bold"}>Add
                Note</Link>
        </main>
    )
}